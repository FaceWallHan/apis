...
/**
 * Servlet implementation class get_roadlight_state
 */
@WebServlet("/get_roadlight_status")
public class get_roadlight_status extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public get_roadlight_status() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		db.getRs("select * from traffic_light where number = '" + RoadLightId + "'");
		ResultSet set = db.getRs();
		try {
			if (set != null&&set.next()) {
				jsonObject2.put("RESULT", "S");
				jsonObject2.put("ERRMSG", "成功");
				jsonObject2.put("Status", set.getString(3));
			} else {
